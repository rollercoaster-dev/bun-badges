 ...
 * 
 * Usage:
 *   bun .cursor/working/utilities/compare-db-environments.js
 */
...
const dbConnections = {
  ...
  test: process.env.TEST_DATABASE_URL || process.env.DATABASE_URL?.replace(/\/([^/]*)$/, '/bun_badges_test'),
};
 ...
 */
async function getSchemaInfo(client, schemaName = 'public') {
  // Tables
...
async function compareEnvironments() {
  ...
  try {
    ...
    const devSettings = devServerInfo.settings.reduce((acc, setting) => {
      acc[setting.name] = setting.setting;
      return acc;
    }, {});
    
    const testSettings = testServerInfo.settings.reduce((acc, setting) => {
      acc[setting.name] = setting.setting;
      return acc;
    }, {});
    ...
    const devExtensions = devServerInfo.extensions.map(ext => `${ext.extname} (${ext.extversion})`).join(', ');
    const testExtensions = testServerInfo.extensions.map(ext => `${ext.extname} (${ext.extversion})`).join(', ');
    ...
    log.compare('Table count', devSchema.tables.length, testSchema.tables.length);
    ...
    const missingInTest = devTableNames.filter(name => !testTableNames.includes(name));
    const missingInDev = testTableNames.filter(name => !devTableNames.includes(name));
    ...
    const tablesToCompare = ['signing_keys', 'badge_assertions', 'badge_classes', 'issuer_profiles', 'status_lists'];
    ...
    for (const tableName of tablesToCompare) {
      ...
      // Compare columns
      const devColumns = devTable.columns.map(c => `${c.column_name} ${c.data_type}${c.character_maximum_length ? `(${c.character_maximum_length})` : ''} ${c.is_nullable === 'YES' ? 'NULL' : 'NOT NULL'}`);
      const testColumns = testTable.columns.map(c => `${c.column_name} ${c.data_type}${c.character_maximum_length ? `(${c.character_maximum_length})` : ''} ${c.is_nullable === 'YES' ? 'NULL' : 'NOT NULL'}`);
      
      const missingColsInTest = devColumns.filter(col => !testColumns.includes(col));
      const missingColsInDev = testColumns.filter(col => !devColumns.includes(col));
      
      if (missingColsInTest.length > 0) {
        log.error(`Columns in dev but missing/different in test for ${tableName}:`, missingColsInTest.join(', '));
      }
      
      if (missingColsInDev.length > 0) {
        log.error(`Columns in test but missing/different in dev for ${tableName}:`, missingColsInDev.join(', '));
      }
      ...
      const missingIdxInTest = devIndexes.filter(idx => !testIndexes.includes(idx));
      const missingIdxInDev = testIndexes.filter(idx => !devIndexes.includes(idx));
      
      if (missingIdxInTest.length > 0) {
        log.error(`Indexes in dev but missing in test for ${tableName}:`, missingIdxInTest.join('\n  '));
      }
      
      if (missingIdxInDev.length > 0) {
        log.error(`Indexes in test but missing in dev for ${tableName}:`, missingIdxInDev.join('\n  '));
      }
    ...
    const getIssuerId = async (client) => {
      const result = await client.query('SELECT issuer_id FROM issuer_profiles LIMIT 1');
    ...
    if (devIssuerId) {
      ...
      try {
        ...
        log.info(`✅ Key query successful in dev, returned ${keyQuery.rows.length} rows`);
      } catch (error) {
        log.error('❌ Key query failed in dev:', error.message);
      }
    }
    ...
    if (testIssuerId) {
      ...
      try {
        ...
        log.info(`✅ Key query successful in test, returned ${keyQuery.rows.length} rows`);
      } catch (error) {
        log.error('❌ Key query failed in test:', error.message);
      }
    }
