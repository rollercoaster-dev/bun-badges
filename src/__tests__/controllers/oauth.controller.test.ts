...
const createMockDatabase = () => ({
  createOAuthClient: mock(() => Promise.resolve({
    id: 'test-client-id',
...
const createMockContext = (options: any = {}) => {
  ...
  return {
    req,
    json: mock((data: any, status?: number): MockResponse => ({ body: data, status })),
...
describe('OAuthController', () => {
  ...
  describe('registerClient', () => {
    it('should register a new client', async () => {
      ...
      const result = await controller.registerClient(ctx as any) as MockResponse;
  ...
  describe('authorize', () => {
    ...
    it('should handle authorization approval', async () => {
      ...
      expect(result.redirect).toContain('https://example.com/callback?code=test-code&state=test-state');
    });
  ...
  describe('token', () => {
    ...
    it('should reject used authorization codes', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('Authorization code has already been used');
      }
    });
