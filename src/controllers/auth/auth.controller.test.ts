...
mock.module("@utils/auth/jwt", () => ({
  ...
  generateToken: async (payload: any) =>
    `mock-${payload.type}-token-${Date.now()}`,
}));
...
describe("Auth Controller", () => {
  ...
  describe(AUTH_ROUTES.VERIFY_CODE, () => {
    ...
    test("returns valid JWT tokens on successful verification", async () => {
      ...
      expect(accessPayload.exp! - accessPayload.iat!).toBeGreaterThanOrEqual(
        900,
      );
      ...
      // Check that expiry is at least 900 seconds
      expect(refreshPayload.exp! - refreshPayload.iat!).toBeGreaterThanOrEqual(
        900,
      );
