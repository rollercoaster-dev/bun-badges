...
describe("Auth Controller", () => {
  ...
  describe(AUTH_ROUTES.VERIFY_CODE, () => {
    ...
    test("returns JWT tokens on successful verification", async () => {
      ...
      const originalGenerate = generateToken;
      const mockGenerateToken = async (payload: any) => {
      ...
      try {
        ...
        // @ts-ignore
        generateToken = originalGenerate;
      }
    });
  ...
  describe(AUTH_ROUTES.REFRESH_TOKEN, () => {
    ...
    test("returns new access token for valid refresh token", async () => {
      ...
      // Mock verifyToken and generateToken
      ...
      const mockGenerateToken = async () => "new-access-token";
