...
describe("API Endpoints", () => {
  ...
  describe("Key Management API", () => {
    test("should create a new key", async () => {
      ...
      // The id is now a UUID generated by the system, not the name we provided
    ...
    test("should get all keys", async () => {
      ...
      // We can't check for the specific key by ID anymore since it's generated
      // Instead, we'll just verify that we have keys in the response
      expect(data.keys.length).toBeGreaterThan(0);
    });
